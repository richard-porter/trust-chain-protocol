...
## Richard Porter (pen name) | Version 0.8 Draft | March 2026
...
**Content-layer complement — Semi-Formal Reasoning (SFR):** TCP’s Context Contamination failure mode identifies the class of attacks that pass chain verification because the input is structurally valid while being semantically poisoned. TCP operates at the authorization layer and cannot detect semantic contamination. Ugare & Chandra’s Semi-Formal Reasoning (arXiv:2603.01896, NeurIPS 2026 candidate) is the first published mechanism for addressing this gap at the content layer. SFR requires agents to construct explicit premises, trace execution paths, and derive formal conclusions. The motivating example — standard reasoning assumes `format()` is Python’s builtin while SFR discovers a module-level override with different semantics — is a textbook Context Contamination scenario. The token passed chain verification. The code was structurally valid. The downstream action was wrong because the semantic premises were false. SFR catches this by forcing the reasoning trace to be explicit and verifiable before conclusions are acted on. SFR is a prompting methodology, not a structural enforcement layer. It improves reasoning quality but does not make incorrect reasoning impossible. This distinguishes it from the Frozen Kernel’s hard constraint approach — SFR is a content-layer complement to TCP’s authorization layer, not a replacement for either. *Confidence grade: Medium — structurally sound mapping; not yet tested in TCP context.*

**Architectural amplification — sink-head position bias:** The “From Spikes to Sinks” mechanistic analysis of Transformer attention sinks suggests a position-dependent amplification mechanism for Context Contamination attacks. Sink heads are biased toward short-range dependencies — they preferentially attend to nearby, recent tokens rather than semantically relevant distant content. In a multi-agent chain where instructions travel across hops, this architectural bias means sink heads will systematically discount distant context in favor of local, recent input. If an adversary injects semantically poisoned content late in a context window — close to the current query position — sink-head geometry creates a structural amplification mechanism. The poisoned content is recent, therefore local, therefore preferentially attended to by the heads most active in shallow processing. This is not a failure of authorization. It is a failure of attention geometry. This is a testable prediction: contamination injected late in the context window should propagate more effectively than contamination injected early, mediated by sink-head attention patterns. The contamination amplification hypothesis is original synthesis from the ecosystem, not a finding of the paper itself. *Confidence grade: Theoretical / Architectural pending experimental confirmation.*
...
**Production analog — model routing as informal delegation:**

Lanham (Medium, February 2026) describes a model routing architecture that functions as a delegation mechanism without formal semantics: a decision engine classifies user requests and routes them to one of several models based on task type, complexity, and cost. The routing decision is undocumented delegation — no scope constraint on what the receiving model may do, no boundary condition specifying when the delegation expires, no audit record on what grounds the routing decision was made, no revocation path if the routed model produces non-compliant output.

TCP’s Delegation Grammar is the formalization of what the Lanham decision engine does informally. The routing layer is a concrete, deployed example of informal delegation in production infrastructure. The gap is not that routing is wrong — it is that routing without delegation semantics creates a class of authorization failures that the router’s own validation framework (win rate, latency, cost, regret rate) cannot detect.

Specifically: the regret rate metric (how often you would override the router’s choice after seeing the output) is the closest thing in the Lanham framework to a governance signal. It is a retrospective quality measurement. TCP’s Chain of Custody and SAFE_PAUSE are prospective governance mechanisms. In a full stack, regret rate data should feed back into FK constraint calibration — high regret on a task category is evidence that the governance envelope for that category is incorrectly specified.

Reference: Lanham, M. “There Is No Best AI Model Anymore — Here’s the Architecture That Replaces It.” Medium, February 2026.
...
|--------------------------------------------|--------------------------------------------------------------------------------------------|-------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|**ASI01 Agent Goal Hijack**                 |Attackers redirect agent objectives via prompt injection or poisoned inputs                 |**Partial — content-layer complement identified**|Chain of Custody verifies authorization chain, not content. Context Contamination (§2.2) names this failure mode. **SFR (Ugare & Chandra, arXiv:2603.01896) is a candidate content-layer complement that substantially strengthens coverage when active alongside TCP.**                                                 |
...
### Promise Theory (Burgess & Bergstra, 2014/2019)

TCP’s Chain of Custody is a promise architecture in the formal sense established by Burgess & Bergstra: each agent commits to a verifiable record of its own behaviour before acting. This is a promise — a unilateral commitment about the agent’s own behaviour — rather than an imposition — an attempt to compel another agent’s behaviour. Promise Theory establishes that only promises, not impositions, are structurally enforceable in autonomous agent systems. The Chain of Custody’s prospective structure (pre-action commitment, not retrospective logging) instantiates the promise architecture: the agent makes the commitment before the action, when the commitment is structurally meaningful, not after, when it is merely a record.

Promise Theory also grounds the TCP Delegation Grammar’s scope limitation: an agent can only delegate promises it has itself made — it cannot delegate authority over another agent’s behaviour it does not control. The subset-only delegation rule (a delegating agent may grant a subset of its own permissions, never a superset) is the Promise Theory unilateral constraint applied to delegation chains.

Promise Theory further distinguishes impositions (attempts to induce compliance in another agent, which cannot be kept by the agent making them) from promises (commitments about one’s own behavior, which are structurally enforceable). Burgess developed Promise Theory specifically because obligation-based frameworks “amounted to wishful thinking” — an obligation imposed from outside assumes compliance without guaranteeing it. The Recitation-Compliance Gap (Diagnostic Vocabulary Entry 23) is the empirical confirmation of this finding in the AI context: a model that has been trained to recite a constraint has received an imposition, not made a promise.

Reference: Burgess, M. & Bergstra, J.A. (2014/2019). *Promise Theory: Principles and Applications.* markburgess.org/promises.html

### Semi-Formal Reasoning / Spikes to Sinks (2026)

Two 2026 papers extend the convergent structural finding to new domains. Ugare & Chandra (arXiv:2603.01896) demonstrate that unstructured chain-of-thought allows semantic confabulation that Semi-Formal Reasoning (SFR) prevents — confirming the content-layer complement to TCP’s authorization-layer coverage and providing a concrete mechanism for the ASI01 gap. The “From Spikes to Sinks” mechanistic analysis provides weight-level evidence that models develop implicit workarounds through optimization pressure in the absence of explicit mechanism — the fifth confirmation of the Frozen Kernel’s foundational architectural claim, and the source of the position-dependent contamination amplification hypothesis in §2.2.
...
|0.8        |March 2026   |Promise Theory (Burgess & Bergstra, 2014/2019) added to Intellectual Lineage — grounds Chain of Custody as promise architecture and Delegation Grammar subset-only rule. SFR content-layer complement (Ugare & Chandra, arXiv:2603.01896) added to Context Contamination failure mode. Sink-head position-dependent amplification hypothesis added to Context Contamination. Lanham informal delegation note added to Delegation Grammar section. ASI01 OWASP row updated with SFR complement. SFR / Spikes to Sinks lineage note added.                                    |
